...
let allCaps = runners.map(function(firstFirst){
    return {first_name: firstFirst.first_name.toUpperCase()}
  });
console.log(allCaps); 
...
let ticketPriceTotal = runners.reduce((acc, cost) => {
    return acc + cost.donation;
  }, 0);
...
let runEmails = runners.map((runner) => {
    return runner.email;
});
console.log(runEmails);

// Problem 2: List names and companies
let companyName = [];
    runners.forEach(runner => companyName.push(runner.first_name + ' from ' + runner.company_name));
console.log(companyName);

// Problem 3: List of guys with small shirths
let = smallShirts = runners.filter(function(sizeM) {
    if (sizeM.shirt_size === 'M') {
      return true;
    }
    return false;
});
